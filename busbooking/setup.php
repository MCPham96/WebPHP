		...
		if(!$form && !$finish){ /* show checks and instructions */
			...
			if(count($checks) && !isset($_POST['test'])){
				...
				<div class="panel panel-warning vspacer-lg">
					<div class="panel-heading"><h3 class="panel-title">C?nh Báo</h3></div>
...
B?n ph?i kh?c ph?c ít nh?t các v?n d? du?c dánh d?u b?ng <i class="glyphicon glyphicon-remove text-danger"></i> tru?c khi ti?p t?c ...</div>
					<?php } ?>
				</div>
				<?php
				if($stop_setup) exit;
	...
	<?php }elseif($finish){ ?>
		...
		<?php
			...
			if(!$_SESSION['adminUsername']){
				?>
				<div id="manual-redir" style="width: 400px; margin: 10px auto;">
N?u không du?c chuy?n hu?ng t? d?ng, <a href="index.php">thì b?n kích vào dây</a>!</div>
				<script>
